...
def get_augmented_probs(model, image, aug_types, n_aug=5):
    ...
    with torch.no_grad():
        for aug in aug_types:
            ...
            aug_probs.append(torch.stack(aug_type_probs))  # shape [n_aug, n_classes]
    return orig_probs, aug_probs  # orig_probs: [n_classes], aug_probs: list of [n_aug, n_classes] for each aug type
...
def adaptive_tta_mix(orig_prob, aug_probs, lam):
    # lam: λ0,λ1,...,λm for m=4 types
    n_aug = aug_probs[0].size(0)  # each type: n_aug augmented versions
    mix = lam[0] * orig_prob
...
def ece_loss_obj(lam_vec):
    lam_full = np.concatenate(([max(0, 1 - lam_vec.sum())], lam_vec))  # λ0 = 1 - sum(λi)
    calibrated_probs = []
    for orig_prob, aug_probs in val_probs_vecs:
        cal_prob = adaptive_tta_mix(orig_prob, aug_probs, lam_full)
        # Top-1 class must remain unchanged
        if cal_prob.argmax().item() != orig_prob.argmax().item():
            return 1e9  # Large penalty for accuracy drop
        #calibrated_probs.append(cal_prob.numpy())
        calibrated_probs.append(cal_prob.detach().numpy())
    return compute_ece(calibrated_probs, np.array(val_labels), n_bins=15)

# Minimize ECE on λ1, λ2, λ3, λ4 (λ0 gets remaining)
#init_lam = np.array([0.05, 0.05, 0.05, 0.05])
#bounds = [(0, 0.5)]*4
#res = minimize(ece_loss_obj, init_lam, method='Nelder-Mead', bounds=bounds)
#best_lam = np.concatenate(([max(0, 1 - res.x.sum())], res.x))

# Add this instead:
init_lam = np.array([1.0, 0.2, 0.2, 0.2, 0.2])  # start with strong original weight
constraints = ({'type': 'eq', 'fun': lambda x: np.sum(x) - 1})
bounds = [(0.0, 1.0)] * 5
res = minimize(ece_loss_obj, init_lam, method='SLSQP', bounds=bounds, constraints=constraints)
best_lam = np.clip(res.x, 0, 1)
...
testset = torch.utils.data.Subset(testset, range(1000, 2000)) # next 1000
...
#    #all_cal_probs.append(cal_prob.numpy())
#    all_cal_probs.append(cal_prob.detach().numpy())


#orig_acc = np.mean(np.array(y_true) == np.array(y_orig))
#cal_acc = np.mean(np.array(y_true) == np.array(y_cal))
#cal_ece = compute_ece(all_cal_probs, np.array(y_true), n_bins=15)
...
# Initial correct and incorrect counts
initial_correct = np.sum(y_true == y_orig)
initial_incorrect = np.sum(y_true != y_orig)

# Correct initially that became incorrect after TTA
correct_to_incorrect = np.sum((y_true == y_orig) & (y_true != y_cal))
# Incorrect initially that became correct after TTA
incorrect_to_correct = np.sum((y_true != y_orig) & (y_true == y_cal))

print(f"Total images correct initially: {initial_correct}")
print(f"Total images incorrect initially: {initial_incorrect}")
...
orig_acc = initial_correct / len(y_true)
cal_acc = np.mean(y_true == y_cal)
cal_ece = compute_ece(all_cal_probs, y_true, n_bins=15)
