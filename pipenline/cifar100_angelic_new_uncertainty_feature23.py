...
def get_restorative_patch(model, image, label, patch_size=8, epsilon=0.1, num_iterations=40):
    ...
    for i in range(num_iterations):
        ...
        with torch.no_grad():
            ...
            _, predicted = patched_output.max(1)
...
if __name__ == "__main__":
    ...
    for image, label in tqdm(testloader, total=len(testloader), desc="Processing All Images"):
        ...
        # Check if the image is unconfident based on entropy
        if entropy.item() > SHANNON_ENTROPY_THRESHOLD:
            ...
            # Generate and apply a new patch for this image
            restorative_patch = get_restorative_patch(model, image, label, patch_size=PATCH_SIZE, num_iterations=10)
