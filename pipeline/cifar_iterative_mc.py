        ...
        if should_refine:
            ...
            while should_refine and count < 10:
                ...
                with torch.no_grad():
                    ...
                    # Re-calculate the MSP for the refinement loop, even if using another metric for thresholding
                    current_msp_for_refinement = torch.max(probabilities_new).item()
                    
                # To avoid an infinite loop and keep the logic similar to your original code
                # we'll refine until the MSP becomes confident
...
def main():
    ...
    # --- Execute your original refinement logic with MC Dropout ---
    print("\n--- Now running dataset refinement process with MC Dropout scores ---")
    
    mc_dropout_threshold = results['mc_dropout']['optimal_threshold']
