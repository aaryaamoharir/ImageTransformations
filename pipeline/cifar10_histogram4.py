...
# --- Helper Functions (Re-factored for clarity) ---
...
def analyze_uncertainty_thresholds(uncertainty_data: Dict[str, np.ndarray], correct_predictions: np.ndarray):
    ...
    for metric_name, values in uncertainty_data.items():
        ...
        # For metrics where higher values indicate more confidence
        is_confidence_metric = metric_name in ['msp']
        
        y_true = 1 - correct_predictions  # 1 for misclassified, 0 for correct
        ...
        try:
            ...
            # Convert back to original scale if it was a confidence metric
...
def main():
    ...
    print("Loading pre-trained CIFAR-10 ResNet-20 model...")
    ...
    print("--- Calculating standard metrics and predictions ---")
    with torch.no_grad():
    ...
    for images, _ in tqdm(testloader_batch, desc="MC Dropout/LU"):
        images = images.to(device)
        mc_dropout_scores, features = calculate_mc_dropout_uncertainty(model, images, NUM_MC_DROPOUT_SAMPLES, feature_layer, device)
    ...
    feature_layer_mahalanobis = model.layer3[2].conv2
